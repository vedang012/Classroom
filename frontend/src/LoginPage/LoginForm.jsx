...
import { loginStudent, loginTeacher } from '../axios/requests';
...
const LoginForm = ({isTeacher}) => {
    ...
    const doLogin = async () => {
        try {
            ...
            const data = isTeacher ? await loginTeacher(email, pass) : await loginStudent(email, pass) 
            console.log(data.data)
            localStorage.setItem("jwt", data.data)
            isTeacher ? navigate("/teacher-dashboard") : navigate("/student-dashboard")
        } catch (error) {
    ...
    return (
        <div className=' w-2/3 mt-4 border border-gray-300 p-3 space-y-5 rounded-md'>
            <h1 className="heading text-center"> {isTeacher ? "Teacher Login" : "Student Login"}</h1>
            <form className='flex flex-col space-y-5'>
                ...
                <div className="flex flex-col">
                    <input
                        type="button"
                        value={`Login as a ${isTeacher ? "Teacher" : "Student"}`}                        
