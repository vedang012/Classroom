...
import { registerStudent } from '../axios/requests';
...
const SignupStudent = () => {
    ...
    const doSignup = async () => {
        try {
            const data = await registerStudent(email, pass)
    ...
    return (
        <div className=' w-2/3 mt-4 border border-gray-300 p-3 space-y-5 rounded-md'>
            <h1 className="heading text-center">Student Signup</h1>
            <form className='flex flex-col space-y-5'>
                ...
                <div className="flex flex-col">
                    <input type="button" onClick={doSignup} value="Signup as a Student" className='Signup-btn rounded-md' />
                </div>
            </form>
        </div>
    )
